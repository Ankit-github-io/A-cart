...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      if (category) {
        link = `${process.env.REACT_APP_BACKEND_URL}/api/v1/products?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
      }
...
export const newReview = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_BACKEND_URL}/api/v1/review/new`,
      reviewData,
      config
    );
    dispatch({
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/api/v1/reviews?id=${id}`
    );
